...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($title) || empty($content)) {
        ...
        if (!empty($_FILES['images']['name'][0])) {
            ...
            foreach ($_FILES['images']['tmp_name'] as $key => $tmp_name) {
                if ($_FILES['images']['error'][$key] == 0) {
                    ...
                    finfo_close($finfo);
                    if (in_array($mime, ['image/jpeg', 'image/png', 'image/gif', 'image/webp']) && $_FILES['images']['size'][$key] <= 10 * 1024 * 1024) {
        ...
        if (!empty($_FILES['video']['name']) && $_FILES['video']['error'] == 0) {
            ...
            finfo_close($finfo);
            if (in_array($mime, ['video/mp4', 'video/webm', 'video/ogg']) && $_FILES['video']['size'] <= 50 * 1024 * 1024) {
    ...
    <div class="container">
        <div class="header">
            ...
            <div class="nav">
                ...
                <a href="profile.php"><?php echo htmlspecialchars($_SESSION['username'] ?? '用户'); ?></a>
