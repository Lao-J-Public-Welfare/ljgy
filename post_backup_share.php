...
<div class="container">
    ...
    <?php if (!empty($post['images']) && $post['images'] != '[]'): ?>
        ...
        <?php foreach ($images as $img): ?>
            <?php if (!empty($img)): ?>
                <img src="/uploads/images/<?php echo $img; ?>" style="max-width:100%; margin:10px 0; border-radius:8px;">
            <?php endif; ?>
        <?php endforeach; ?>
    <?php endif; ?>
    
    <?php if (!empty($post['video'])): ?>
        <video controls style="max-width:100%; margin:10px 0; border-radius:8px;">
            <source src="/uploads/videos/<?php echo $post['video']; ?>">
        </video>
    <?php endif; ?>
</div>
        <div class="post-actions">
            <span class="like-btn" data-id="<?php echo $post['id']; ?>" data-type="post">
                点赞 <span class="like-count"><?php echo $post['like_count'] ?? 0; ?></span>
            </span>
            <span class="share-btn" data-url="<?php echo 'https:
                分享
            </span>
        </div>
    </div>
    ...
    <div class="comment-section">
        ...
        <div class="comment-form">
            <form method="POST">
                <textarea name="content" placeholder="发表你的评论..." required></textarea>
...
function generateShortLink(postId) {
    fetch("/short/create.json?id=" + postId + "&type=post")
        .then(res => res.json())
        .then(data => {
            if (data.url) {
